...
- **P7.4 — image gen fully through AI Gateway** (commit `<TBD>`). Primary `bfl/flux-2-flex`, fallback `google/gemini-3-pro-image`. AI SDK `experimental_generateImage` via `@ai-sdk/gateway`. Returns base64 data URLs (caller persists to Convex File Storage). `openai` and `@fal-ai/client` deps removed; `openaiKey` + `falKey` dropped from schema/users/BYOK form/run-context. RunKeys is now 7 fields (was 9).
